...
| Dreamer scheduler + journal (2026-07-26) | Dreamer runs inside the daemon behind a `Pass` interface (`Run(ctx, budget) → action lines`). Scheduled cycles are gated on vault idle + inbox drained + interval elapsed; per-pass action budget bounds work; one cycle at a time. `mnemo dream` (`POST /dream`) always available for manual supervised cycles; background scheduling config-gated, default **off** until the Phase 2.5 external-drive remote lands. All cycle actions journaled to monthly `archive/dreamer-journal-YYYY-MM` notes. | Idle gating keeps the dreamer from racing the filer or a mid-edit human; budgets bound per-cycle LLM cost. Journal placed in `archive/`: it is audit trail like processed captures — in the vault so it survives with the notes, excluded from search/embeddings so it never pollutes retrieval. Default-off honours the 2.5 redundancy gate while manual cycles stay usable. `POST /dream` sits on the shared bearer surface — added to the parked Admin surface auth item. |
| Linker design (2026-07-30) | Three stages in one pass: embedding-similar candidate pairs (note↔note only — hub hits skipped, hub membership is the filer's job; symmetric dedupe via sorted pair keys; already-linked-either-direction filtered; score floor 0.3; budget = N best by score), LLM judge (one chat call per pair, no tools; JSON schema enforced via ollama structured outputs `format`, so replies always parse; unparseable still fails safe to skip), then writes through `Store.Link` — a first-class locked, idempotent store mutation appending `- [[to]] — reason` under `## Related`, mirroring `AddToHub`. The judge picks link placement via an `into` field whose schema enum is exactly the two slugs; links are one-sided (backlink index covers the reverse). Skip verdicts cached in an in-memory judged set for the daemon lifetime. Verdict schema order is `deliberation → link → into → reason` — deliberation is unstored scratch space generated before the verdict; reason is the stored annotation, generated last so it is written knowing verdict and placement, addressed to the `into` note's reader. Reason is sanitized (wikilink brackets stripped) before writing. | Similarity score doesn't predict link-worthiness (dry runs: top score 0.80 was a skip, the sole link sat at 0.65) — a threshold can't replace the judge; candidates are recall, the judge is precision. Default-no prompt because LLMs over-link; judge criterion is reader benefit for the lazy LLM consumer, not topical relatedness (embeddings already answered that). Separate deliberation and reason fields because one field can't do both jobs: chain-of-thought must precede the verdict, an annotation must follow it — when a single reason field came first (run 4), the model crammed verdict words and slugs into it and occasionally contradicted its own `link` boolean. In-memory judged set over persistence: dry run showed ~95% skips re-judged every cycle, but a restart re-judging lets stale skips heal as notes grow, and the vault stays the only durable truth. Floor 0.3 from observed data: everything below was skipped (0.11 pairs burned GPU confirming risotto ≠ Neovim). |
...
Work split (agreed 2026-07-26, optimised for concept-learning value): Cameron owns the passes with conceptual depth — the linker (embedding-space candidates + LLM judge) and the consolidator (design + prompts; Claude pairs on the destructive-write safety rails). Claude owns the plumbing: scheduler, journal, gardener, hub maintenance. Background scheduling ships **disabled by default** (`[dreamer] enabled = false`) — do not enable until the Phase 2.5 external-drive remote exists; `mnemo dream` gives supervised manual cycles meanwhile.

- [x] Scheduler ✅ (2026-07-26): idle-gated cycles (vault quiet for `idle_minutes`, inbox drained, `interval_minutes` between cycles), per-pass action budget (`pass_budget`), one cycle at a time; `mnemo dream` / `POST /dream` trigger a cycle manually and return per-pass reports; dreamer state in `/status`
- [x] Journal ✅ (2026-07-26): every cycle's actions appended to a monthly `archive/dreamer-journal-YYYY-MM` note — in the vault so the audit trail survives with the notes, in `archive/` so it stays out of search and embeddings (same reasoning as processed captures); quiet cycles write nothing
- [x] Gardener ✅ (2026-07-26): deterministic hygiene — repairs safe frontmatter (missing `type: hub`), reports what needs intelligence: missing descriptions, broken links, notes in no hub, inbox stragglers >24h
- [x] Linker (Cameron) ✅ (2026-07-30): vector-similar candidate pairs (hub hits and scores <0.3 excluded) → LLM judge per pair, single schema-constrained chat call → LINK verdicts written via `Store.Link` into the judge-chosen note's `## Related` section; skip verdicts remembered in-memory for the daemon's lifetime
- [ ] Hub maintenance (Claude): new/orphan notes added to hubs, stale descriptions rewritten, oversized hubs split
- [ ] Consolidator (Cameron design + prompts, Claude write safety; last, once the journal gives it a safety rail): detect duplicates/overlaps → merge or cross-link; contradiction flagged in-note
...
- [ ] Conversational queries: messages routed to the agent loop with vault tools; answers back in chat (Cameron — reserved as the build-an-agent-loop-from-scratch exercise)
- [ ] Filing confirmations ("filed under [[x]], linked to [[y]]") + correction replies

### Phase 6 — Learning loop

Reserved for Cameron (2026-07-26): self-learning is a core learning objective — design together when the phase starts.
